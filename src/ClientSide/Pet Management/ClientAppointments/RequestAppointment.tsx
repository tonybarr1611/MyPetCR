  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      if (!appointment.IDPet || !appointment.IDStore || !appointment.DateTime) {
        ...
        await axios.post(
          "http://localhost:8080/api/v1/appointmentandinvoice",
          appointmentData
        );
        logger.request(
          "User has requested an appointment for pet with ID " +
            appointmentData.IDPet +
            " at store with ID " +
            appointmentData.IDStore +
            " at date/time " +
